...
def to_netcdfMint(ds, info, dataset_type, bounding_box, dir_out):
    ...
    if info['index'] == 'SPI':
        ...
        ds['title'] = long_name
        ds['summary'] = info['periodicity']+' standardized precipitation index inferred from '\
        ...
        ds['title'] = long_name
        ds['summary'] = 'Monthly potential evapotranspiration inferred from '\
        + dataset_type +'.'
        info['periodicity']='monthly'
    elif info['index'] == 'SPEI':
        long_name = 'Standardized Precipitation Evapotranspiration Index'
        ds['title'] = long_name
        ds['summary'] = info['periodicity']+' standardized precipitation  evapotranspiration index inferred from '\
        + dataset_type +', using a calibration period from '+\
        str(info['calibration_start']) + ' to '+ str(info['calibration_end'])+\
            ' using a '+ info['distribution']+ ' distribution with a '+\
            str(info['timescales'])+ '-month timescale.'
...
#Test
#dataset_type = 'GLDASv2.0'
#index = 'PET'
#dataset_name = 'GLDASv2.0' #file name or directory name
